...
DEFAULT_REPEAT_REL = r"风机联合连续相同检测\联合重复值检测结果.xlsx"
...
# 构建全站冻结分段
# ============================================================
def build_segments_for_station(df_repeat: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
...
def append_segment_anomaly_stats(segment_df: pd.DataFrame, minute_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if minute_df is None or len(minute_df) == 0:
        ...
        stat_cols = [
            ...
            "是否异常分钟减少", "是否异常程度下降", "是否全部修正为正常",
        ]
        for col in stat_cols:
            out[col] = 0
        return out

    idx = pd.DatetimeIndex(minute_df["时间"]).sort_values()
    ...
    fill_zero_cols = [
        "原是否异常", "是否异常", "原异常程度", "异常程度", "原负损耗", "修正后负损耗",
    ...
    # 异常程度统计只对纳入汇总统计的分钟计算，避免物理越限极端值拉爆。
...
def main():
    ...
    parser.add_argument("--station-dir", default="场站数据\LGXRFD", help="场站目录")
    parser.add_argument("--step2-dir", default="场站数据\LGXRFD\step2-异常检测", help="STEP2 输出目录，包含 station_anomaly_detail.csv")
    parser.add_argument("--output-dir", default="场站数据\LGXRFD\step3-重复冻结分段", help="STEP3 输出目录")
    ...
    segment_df, detail_df = build_segments_for_station(repeat_df)
