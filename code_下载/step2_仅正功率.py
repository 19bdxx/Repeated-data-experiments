...
DEFAULT_FAN_JOINT_REPEAT_REL = os.path.join("风机联合连续相同检测_仅正功率", "联合重复值检测结果.xlsx")
...
def compute_quantile_track(
    ...
    异常程度：
        只对越出正常带的部分计算，正常带内为 0。
        用于衡量“越界异常有多严重”。

    偏离程度：
        正常带内外都计算。
        普通功率区间：abs(L - Q50) / max(Q95-Q05, floor)
        超低功率区间：abs(L - P) / max(P-0.99P, floor)
        用于衡量“距离经验中心/理想损耗有多远”。
    """
    ...
    q_low_col = quantile_col_name(lower_pct)
    q_high_col = quantile_col_name(upper_pct)
    q_mid_col = quantile_col_name(50)
    ...
    if q_mid_col in empirical_df.columns:
        q_mid = interp_1d_with_hold(p, empirical_df["P_med"].values, empirical_df[q_mid_col].values)
    else:
        # 兜底：如果 STEP1 未输出 Q50，则用上下界中点近似中心。
    ...
    center[ultra_mask] = p[ultra_mask]

    valid = np.isfinite(p) & np.isfinite(loss) & np.isfinite(lower) & np.isfinite(upper) & np.isfinite(center)
    ...
    # 偏离程度：正常带内外均计算，用于衡量修正后是否更接近正常中心/理想值。
    deviation_amount = np.where(valid, np.abs(loss - center), np.nan)
    deviation = np.where(valid, deviation_amount / denom, np.nan)
...
def build_detail_df(df: pd.DataFrame, station_name: str) -> pd.DataFrame:
    ...
    out["原经验中位数或理想损耗MW"] = df.get("原经验中位数或理想损耗MW", np.nan)
    ...
    out["原偏离量MW"] = df.get("原偏离量MW", np.nan)
    out["原偏离程度"] = df.get("原偏离程度", np.nan)

    out["修正后经验带下界MW"] = df.get("修正后经验下界MW", np.nan)
    out["修正后经验带上界MW"] = df.get("修正后经验上界MW", np.nan)
    out["修正后经验中位数或理想损耗MW"] = df.get("修正后经验中位数或理想损耗MW", np.nan)
    ...
    out["修正后偏离量MW"] = df.get("修正后偏离量MW", np.nan)
    out["修正后偏离程度"] = df.get("修正后偏离程度", np.nan)
...
def build_summary_df(df: pd.DataFrame, station_name: str, args: argparse.Namespace) -> pd.DataFrame:
    ...
    风机功率之和等物理越限点只在明细中保留，不参与本汇总的异常率、损耗、异常程度和偏离程度统计，
    避免极端值把汇总指标拉得失真。

    注意：
    - 异常程度：只统计异常点的越界严重程度。
    - 偏离程度：统计所有纳入统计的有效点，用于评估整体是否更接近经验中心/理想损耗。
    """
    total_rows = len(df)
    ...
    raw_valid_mask = stat_df["原是否有效检测"].fillna(False).astype(bool) if stat_rows else pd.Series(dtype=bool)
    corr_valid_mask = stat_df["修正后是否有效检测"].fillna(False).astype(bool) if stat_rows else pd.Series(dtype=bool)

    raw_valid = int(raw_valid_mask.sum()) if stat_rows else 0
    corr_valid = int(corr_valid_mask.sum()) if stat_rows else 0
    ...
    raw_dev = pd.to_numeric(stat_df.get("原偏离程度", pd.Series(dtype=float)), errors="coerce")
    corr_dev = pd.to_numeric(stat_df.get("修正后偏离程度", pd.Series(dtype=float)), errors="coerce")
    raw_dev_valid = raw_dev[raw_valid_mask].dropna() if stat_rows else pd.Series(dtype=float)
    corr_dev_valid = corr_dev[corr_valid_mask].dropna() if stat_rows else pd.Series(dtype=float)
    ...
    raw_dev_sum = float(raw_dev_valid.sum()) if len(raw_dev_valid) else 0.0
    corr_dev_sum = float(corr_dev_valid.sum()) if len(corr_dev_valid) else 0.0
    ...
    row = {
        ...
        "原异常程度总和（异常点，纳入统计）": float(raw_anom_sev.sum()),
        "修正后异常程度总和（异常点，纳入统计）": float(corr_anom_sev.sum()),
        ...
        "原偏离程度总和（有效点，纳入统计）": raw_dev_sum,
        "修正后偏离程度总和（有效点，纳入统计）": corr_dev_sum,
        "偏离程度下降量（原-修正后）": raw_dev_sum - corr_dev_sum,
        "原平均偏离程度（有效点）": float(raw_dev_valid.mean()) if len(raw_dev_valid) > 0 else 0.0,
        "修正后平均偏离程度（有效点）": float(corr_dev_valid.mean()) if len(corr_dev_valid) > 0 else 0.0,
        "原最大偏离程度（有效点）": float(raw_dev_valid.max()) if len(raw_dev_valid) > 0 else 0.0,
        "修正后最大偏离程度（有效点）": float(corr_dev_valid.max()) if len(corr_dev_valid) > 0 else 0.0,
        ...
        "异常程度/偏离程度带宽下限MW": args.severity_bandwidth_floor,
    ...
    # 兼容旧字段名，便于后续旧 STEP3/STEP4 暂时仍可读取。
    row["原异常程度总和（纳入统计）"] = row["原异常程度总和（异常点，纳入统计）"]
    row["修正后异常程度总和（纳入统计）"] = row["修正后异常程度总和（异常点，纳入统计）"]
    row["异常程度带宽下限MW"] = args.severity_bandwidth_floor
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--station-dir", default=r"场站数据\JMZSFD", help="场站目录，内部应包含主 CSV 和重复值检测结果子目录")
    parser.add_argument("--model-dir", default=r"场站数据\JMZSFD\step1-插值损耗模型-仅正功率",  help="STEP1 输出目录，包含 fit_model_summary.csv / fit_model_interp_points.csv")
    parser.add_argument("--output-dir", default=r"场站数据\JMZSFD\step2-异常检测-仅正功率",  help="STEP2 输出目录")
...
def main() -> None:
    ...
    if len(summary_df) > 0:
        ...
        if '原偏离程度总和（有效点，纳入统计）' in row.index:
            print(f"  原偏离程度总和: {float(row['原偏离程度总和（有效点，纳入统计）']):.4f}")
            print(f"  修正后偏离程度总和: {float(row['修正后偏离程度总和（有效点，纳入统计）']):.4f}")
            print(f"  偏离程度下降量: {float(row['偏离程度下降量（原-修正后）']):.4f}")
