...
DEFAULT_STEP2_DETAIL = r"场站数据\JMZSFD\step2-异常检测-仅正功率\station_anomaly_detail.csv"
DEFAULT_STEP4_SCORED = r"场站数据\JMZSFD\step4-阈值分析-细分版-仅正功率\threshold_scan_scored.csv"
...
def create_app(detail_df: pd.DataFrame, scored_df: pd.DataFrame, max_points_default: int) -> Dash:
    ...
    display_categories = [f"原始｜{x}" for x in RAW_DISPLAY_SUBTYPES] + [f"修正后｜{CORRECTED_SUBTYPE}"]
    category_options = [{"label": x, "value": x} for x in display_categories]

    app.layout = html.Div(
        style={"fontFamily": "Arial, sans-serif", "padding": "16px"},
        children=[
            html.H2("STEP4 阈值结果：原始5类 + 修正后1类 功率-损耗散点图"),
            html.Div(
                style={"display": "grid", "gridTemplateColumns": "220px 520px 220px 220px", "gap": "12px", "marginBottom": "12px"},
                children=[
                    html.Div([
                        html.Label("阈值"),
            ...
            html.Div(
                "说明：前5类使用原始风机功率之和/原损耗绘制；第6类仅针对“重复正功率>0｜修正后保留”，使用修正后风机功率之和/修正后损耗绘制。这样可以直接对比修正前后落点变化。",
    ...
    def update_graph(threshold_value, display_categories_selected, include_mode, size_col, max_points):
        ...
        corr_plot["图中显示类别"] = f"修正后｜{CORRECTED_SUBTYPE}"
        corr_plot["图中口径"] = "修正后"
        ...
        fig = px.scatter(
            ...
            title=f"阈值={th:.4g} | 原始5类 + 修正后1类 | 横坐标=风机功率之和，纵坐标=损耗",
        )
        ...
        metrics = (
            ...
            f"构造后6类候选点数={before_xy_filter:,} | 有效XY点数={total_before_sample:,} | "
            f"实际绘制点数={len(plot_df):,} | {sampling_text} | XY无效丢弃点数={dropped_xy:,} | "
            f"平均异常程度={avg_sev:.4f} | 平均偏离程度={avg_dev:.4f} | {count_text}"
        )
        return fig, metrics
...
def main():
    parser = argparse.ArgumentParser(description="Dash：原始5类 + 修正后1类的风机功率-损耗散点图")
