...
输出：
    ...
    turbine_predictions_by_fan.csv（可选）
    curtail_summary.csv
    sample_summary.csv
    loss_curve_used.csv
"""
...
def parse_feature_sets(s: str) -> List[str]:
    ...
    for x in str(s).replace("，", ",").split(","):
        ...
        if not x:
            continue
        if x not in allowed:
            raise ValueError(f"未知 feature_set: {x}，仅支持 {sorted(allowed)}")
        out.append(x)
    return out


def get_station_status_feature_cols(df: pd.DataFrame, feature_set: str) -> List[str]:
    """
    全站模型状态特征。
    ...
    power_status:
      ...
      不使用 COUNT，避免和可用分钟数/风机数强相关。
    """
    if feature_set == "power_only":
        return []

    cols = []
    for c in df.columns:
        s = str(c)
        if re.match(r"^STATION_STATUS_RATIO_[-]?\d+_15MIN$", s):
            cols.append(s)
        elif re.match(r"^STATION_STATUS_LAST_RATIO_[-]?\d+_15MIN$", s):
            cols.append(s)
    return sorted(cols)


def get_turbine_status_feature_cols(df: pd.DataFrame, fan_no: int, model_name: str, feature_set: str) -> List[str]:
    """
    单机模型状态特征。
    ...
    power_status:
      使用该风机的 STATUS_COUNT_<code>_#n_15MIN。
      对树模型额外使用 STATUS_LAST_#n_15MIN。
      对 ridge 不使用 STATUS_LAST，因为状态码本身是类别，直接当连续数字不太合理。
    """
    if feature_set == "power_only":
        return []

    cols = []
    # 状态计数列：适合所有模型，包括 ridge。
    count_pattern = re.compile(rf"^STATUS_COUNT_[-]?\d+_#{fan_no}_15MIN$")
    for c in df.columns:
        if count_pattern.match(str(c)):
            cols.append(str(c))

    # 树模型可以直接使用最后状态码作为数值型类别提示。
    if str(model_name).lower() in {"lightgbm", "xgboost", "random_forest"}:
        last_col = f"STATUS_LAST_#{fan_no}_15MIN"
...
def extract_fan_no_from_power_col(col: str) -> int:
...
def append_metrics(
    ...
    for split in ["val", "test"]:
        ...
        rows.append({
            "model": model_name,
            "feature_set": str(pred_df["feature_set"].iloc[0]) if "feature_set" in pred_df.columns and len(pred_df) > 0 else "power_only",
...
def build_bias_diagnostics(predictions_all: pd.DataFrame, rated_power_mw: float) -> pd.DataFrame:
    """
    诊断1：检查不同方案是否存在系统性高估/低估。

    bias = mean(pred_station_power - true_station_power)
    bias > 0 表示高估；bias < 0 表示低估。
    """
    rows = []
...
def extract_fan_no_from_col(col: str) -> int:
    m = re.search(r"#(\d+)", str(col))
    return int(m.group(1)) if m else -1
...
def build_feature_gain_summary(metrics_summary: pd.DataFrame) -> pd.DataFrame:
    """
    对比 power_status 相对 power_only 的提升。

    MAE_improvement = MAE_power_only - MAE_power_status
    >0 表示加入状态码后 MAE 降低。
    """
    ...
    p = tmp[tmp["feature_set"].eq("power_only")].copy()
    s = tmp[tmp["feature_set"].eq("power_status")].copy()

    merged = p.merge(
        s,
        on=key_cols,
        how="inner",
        suffixes=("_power_only", "_power_status"),
    )
    if len(merged) == 0:
        return merged
...
def run_station_direct(
    ...
    split_val_time: pd.Timestamp,
    split_test_time: pd.Timestamp,
    feature_set: str = "power_only",
) -> pd.DataFrame:
    feature_cols = [c for c in STATION_FEATURE_COLS if c in df.columns]
    if feature_set == "power_status":
        feature_cols = feature_cols + get_station_status_feature_cols(df, feature_set)
...
def run_turbine_models(
    ...
    """
    每台风机单独训练模型，返回：
    - sum_pred_df：timestamp/split/pred_fan_sum_native
    - turbine_sample_summary：每台风机样本数统计
    - turbine_metrics_by_fan：每台风机预测效果指标
    - turbine_predictions_by_fan：每台风机预测明细；可选输出，默认可为空
    """
    pred_parts = []
    sample_rows = []
    metric_rows = []
    ...
    for idx, col in enumerate(fan_cols, start=1):
        fan_no = extract_fan_no_from_col(col)
        try:
            fan_no = extract_fan_no_from_power_col(col)
            feature_cols = [col]
            if feature_set == "power_status":
                feature_cols = feature_cols + get_turbine_status_feature_cols(df, fan_no, model_name, feature_set)
            feature_cols = list(dict.fromkeys(feature_cols))

            ds = build_supervised_dataset(df, feature_cols, col, M, N, freq_minutes, split_val_time, split_test_time)
            ...
            # 每台风机指标：val/test 分开统计
            for split in ["val", "test"]:
                sub = pred[pred["split"] == split]
                metric = calc_one_metric_row(
                    sub["true_value"],
                    sub[pred_col],
                    rated_power=fan_capacity_mw,
                )
                metric_rows.append({
                    "fan_no": fan_no,
                    "fan_col": col,
            ...
            pred_for_sum = pred[["timestamp", "split", pred_col]].copy()
            pred_parts.append(pred_for_sum)
        except Exception as exc:
            print(f"  ⚠️ 风机 {col} 训练失败：{exc}")
            sample_rows.append({"fan_no": fan_no, "fan_col": col, "feature_set": feature_set, "error": str(exc)})
    ...
    # 当前仍使用内连接：只有所有成功风机都有预测的时刻才参与求和。
    # 诊断输出会帮助判断是否由少数风机样本不足/预测差导致汇总效果受影响。
    merged = pred_parts[0]
    ...
    merged["pred_turbine_coverage"] = len(pred_cols) / max(len(fan_cols), 1)

    sum_pred = merged[["timestamp", "split", "pred_fan_sum_native", "pred_turbine_count", "pred_turbine_coverage"]].copy()
...
def main() -> None:
            ...
            for N in n_list:
                ...
                try:
                    ...
                    fan_sum_pred, fan_sample_summary, fan_metrics, fan_detail = run_turbine_models(
                        ...
                        fan_capacity_mw=args.fan_rated_power_mw,
                    ...
                    fan_sample_summary["horizon_minutes"] = N * args.freq_minutes
                    ...
                    all_predictions.append(pred_raw[[
                        ...
                        "pred_fan_sum_native", "pred_fan_sum_mw", "pred_turbine_count", "pred_turbine_coverage", "scheme", "model", "feature_set", "horizon_steps", "horizon_minutes", "error", "abs_error"
                    ]])
                    ...
                    all_predictions.append(pred_loss[[
                        "timestamp", "split", "true_station_power", "pred_station_power",
                        "pred_fan_sum_native", "pred_fan_sum_mw", "pred_turbine_count", "pred_turbine_coverage", "pred_loss_q50",
    ...
    if turbine_metrics_by_fan is not None and len(turbine_metrics_by_fan) > 0:
        turbine_metrics_by_fan.to_csv(turbine_metrics_path, index=False, encoding="utf-8-sig")
    if turbine_predictions_by_fan is not None and len(turbine_predictions_by_fan) > 0:
    ...
    if turbine_metrics_by_fan is not None and len(turbine_metrics_by_fan) > 0:
        print("\n每台风机预测效果最差Top10（按test MAE）：")
        worst = turbine_metrics_by_fan[turbine_metrics_by_fan["split"].eq("test")].copy()
        if len(worst) > 0:
            cols = ["model", "feature_set", "horizon_steps", "fan_no", "fan_col", "sample_count", "MAE", "RMSE", "R2", "bias_MW", "NMAE_by_fan_capacity"]
            cols = [c for c in cols if c in worst.columns]
            print(worst.sort_values("MAE", ascending=False)[cols].head(10).to_string(index=False))
