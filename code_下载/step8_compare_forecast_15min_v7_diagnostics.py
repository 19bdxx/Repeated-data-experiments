...
DEFAULT_STEP8_DIR_NAME = "step8-预测实验_V7"
...
def build_bias_diagnostics(predictions_all: pd.DataFrame, rated_power_mw: float) -> pd.DataFrame:
    ...
    """
    rows = []
    required = {"model", "horizon_steps", "horizon_minutes", "scheme", "split", "true_station_power", "pred_station_power"}
    if not required.issubset(predictions_all.columns):
        return pd.DataFrame()

    group_cols = ["model", "horizon_steps", "horizon_minutes", "scheme", "split"]
...
def run_turbine_models(
    ...
    for idx, col in enumerate(fan_cols, start=1):
        ...
        try:
            ...
            sample_rows.append({"fan_no": fan_no, "fan_col": col, "error": str(exc)})
...
def main() -> None:
        ...
        for N in n_list:
            ...
            try:
                ...
                all_predictions.append(pred_raw[[
                    ...
                    "pred_fan_sum_native", "pred_fan_sum_mw", "pred_turbine_count", "pred_turbine_coverage", "scheme", "model", "horizon_steps", "horizon_minutes", "error", "abs_error"
                ]])
    ...
    if turbine_metrics_by_fan is not None and len(turbine_metrics_by_fan) > 0:
        ...
        if len(worst) > 0:
            cols = ["model", "horizon_steps", "fan_no", "fan_col", "sample_count", "MAE", "RMSE", "R2", "bias_MW", "NMAE_by_fan_capacity"]
