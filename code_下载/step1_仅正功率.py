...
def load_fan_joint_anomaly_segments(
    station_dir: str,
    fan_joint_repeat_rel: str = r"风机联合连续相同检测_仅正功率\联合重复值检测结果.xlsx",
) -> pd.DataFrame:
    """读取风机联合连续相同检测结果。"""
    anomaly_file = fan_joint_repeat_rel
    if not os.path.isabs(anomaly_file):
        anomaly_file = os.path.join(station_dir, fan_joint_repeat_rel)

    if not os.path.exists(anomaly_file):
        raise FileNotFoundError(f"未找到风机联合重复异常段文件：{anomaly_file}")

    anom_df = pd.read_excel(anomaly_file, engine="openpyxl")
    print(f"  ✅ 已读取风机联合重复异常段文件：{anomaly_file}（{len(anom_df):,} 条）")
...
def load_anomaly_segments(
    ...
    fan_joint_df = load_fan_joint_anomaly_segments(
        station_dir=station_dir,
        fan_joint_repeat_rel=fan_joint_repeat_rel,
    )
...
def main():
    ...
    parser.add_argument(
        "--station-dir",
        type=str,
        default=r"G:\WindPowerForecast\#1场站数据下载\代码-从日志提取\江苏\场站数据\LGXRFD",
        help=r"场站目录，例如 G:\WindPowerForecast\#1场站数据下载\代码-从日志提取\江苏\场站数据\LGXRFD"
    )
    parser.add_argument("--output-dir", type=str, default=r"场站数据\LGXRFD\step1-插值损耗模型-仅正功率", help="输出目录")
    parser.add_argument(
        "--fan-joint-repeat-rel",
        type=str,
        default=r"风机联合连续相同检测_仅正功率\联合重复值检测结果.xlsx",
        help="风机联合重复检测结果相对 station-dir 的路径；也可以传绝对路径",
    )
    ...
    anom_df = load_anomaly_segments(
        ...
        enable_station_repeat_hard_anomaly=enable_station_repeat_hard_anomaly,
        fan_joint_repeat_rel=args.fan_joint_repeat_rel,
    )
